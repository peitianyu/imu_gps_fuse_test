...
#include "common/gps/gps_convertor.h"
#include "common/imu/imu.h"
#include "common/imu/imu_preintegrator.h"

// 外参标定: extrinsic parameters calibration
class ImuGpsLocalizer
{
public:
    ImuGpsLocalizer(const Eigen::Vector3d& calib_extr, const ImuPreintegrator::Option& imu_option)
        : is_inited_(false), calib_extr_(calib_extr), fuse_state_(State()),
          imu_preintegrator_(std::make_unique<ImuPreintegrator>(imu_option)){}
          

    bool ImuPredict(const Imu& imu)
    {
        if(!is_inited_) return false;

        static Imu last_imu = imu;
    ...
    void GpsUpdate(const Gps& gps)
    {
        // FIXME: 实际上还有一个时间戳对齐, 这里暂时不考虑
        if(!is_inited_){
            is_inited_ = true;
            gps_convertor_ = std::make_unique<GpsConverter>(gps.lla);
            return ;
        }
    ...
    State GetFuseState() { return fuse_state_; }
private:
    void UpdateStateByGps(const Gps& gps)
    {
    ...
    void ComputeJacobianAndResidual(const Gps& gps, const State& state, const std::unique_ptr<GpsConverter>& gps_convertor, 
    ...
    void UpdateState(const Eigen::VectorXd& delta_x, State& state)
    {
...
private:
    ...
    std::unique_ptr<GpsConverter> gps_convertor_;
};
