...
static void (*g_gps_cb)(const Gps&) = nullptr;
void RegisterGpsCallback(void(*cb)(const Gps&)){
    g_gps_cb = cb;
}

static void (*g_imu_cb)(const Imu&) = nullptr;
...
void ReadData(const std::string& file_path)
{
    ...
    if(type == "imu"){
        ...
        g_imu_cb((Imu){time_stamp, Eigen::Vector3d(acc_x, acc_y, acc_z), Eigen::Vector3d(gyr_x, gyr_y, gyr_z)});
    }
    else if(type == "gps"){
        double time_stamp;
        double lat, lon, alt;
        ...
        g_gps_cb((Gps){time_stamp, Eigen::Vector3d(lat, lon, alt), cov});
    }
    else{
